...
reply(`
لا تمنشنه لانه في وضع الاختفاء الان ${reason ? 'with reason ' + reason : 'بدون سبب'}
...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    const menulist = `
    Konichiwa ${pushname} dear 👋. I am/انا العزيز ${global.BotName}, a bot developed by: Raizel to take your WhatsApp usage into next level/روبوت تم تطويره بواسطة: رايزل للانتقال باستخدام واتساب إلى المستوى التالي.
...
replay('⚡تم * رفع الحظر * عن هذه المجموعة  الأن يمكنكم الإستمتاع!')
} else {
  let buttonsntnsfw = [
...
case 'قائمه-الاباحي':
    ...
    if (!AntiNsfw) return reply(mess.nonsfw)
        reply(` *━━━〈  📛 قائمه الاباحي 📛  〉━━━*\n\nمافي اباحي ولا سي يعم صلي علي النبي في قلبك واتركك من اللي تسويه هذا ⚡😇️`)
    break
...
if (isBanChat) return reply(mess.bangc)
				   ...
				   reply(mess.waiting)
				  Miku.sendMessage(m.chat, { image: { url: cok }, caption: 'تفضل أيها المغامر/ة⚡♥...' }, { quoted: m })
				  break
...
let mese = await Miku.sendMessage(m.chat, {image:{url:emoji.images[4].url}, caption: `تفضل أيها الغامر/ة⚡♥...`}, {quoted:m})
...
await Miku.sendMessage(m.chat, { delete: key })
 ...
 replay('⚡تم تفعيل منع جميع الروابط تحذير⚡ !')
 var groupe = await Miku.groupMetadata(from)
 var members = groupe['participants']
 var mems = []
 members.map(async adm => {
 ...
 if (!AntiLinkAll) return replay('⚡تم إلغاء تنشيطه بنجاج⚡')
 ...
 if (!antiWame) return replay('⚡تم إلغاء تنشيطه بنجاح⚡')
 ...
 if (!AntiNsfw) return replay('⚡تم إلغاء تنشيطه بنجاح⚡')
...
let mentioned = participants.map(v => v.jid)
 ...
 case 'resetlinkgc':
    ...
    case 'group': case 'جروب': case 'قروب': {
     ...
     if (!isAdmins && !isCreator) return replay(mess.useradmin)
     if (args[0] === 'close'){
     await Miku.groupSettingUpdate(m.chat, 'announcement').then((res) => replay(`⚡تم إغلاق المجموعة بنجاح⚡!`)).catch((err) => replay(jsonformat(err)))
     } else if (args[0] === 'open'){
     await Miku.groupSettingUpdate(m.chat, 'not_announcement').then((res) => replay(`⚡تم فتح المجموعة بنجاح⚡!`)).catch((err) => replay(jsonformat(err)))
     } else {
     let buttons = [
...
case 'لفيديو': case 'tovideo': {
 ...
 await Miku.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'تفضل أيها المغامر/ة⚡♥...' } }, { quoted: m })
...
if (isBanChat) return reply(mess.bangc)
if (args[0] === "mp4") {
Miku.sendMessage(from, {video:{url:args[1]}, caption:'تفضل أيها المغامر/ة⚡♥...', mimetype:'video/mp4'}, {quoted:m})
} else if (args[0] === "jpg") {
Miku.sendMessage(from, {image:{url:args[1]}, caption:'تفضل أيها المغامر/ة⚡♥...'}, {quoted:m})
} else {
reply("Error! ")
}
}
...
case 'fbddlxx': {
 ...
 if (isBanChat) return reply(mess.bangc)
 let buttons = [
 {buttonId: `-menu`, buttonText: {displayText: 'الأوامر'}, type: 1}
 ]
 let buttonMessage = {
 video: {url:args[0]},
 caption: "Done!",
...
انا *بوتة نويل*, 
تم تطويره بواسطة: *دابي سان*.

⚡🌊 الأوامر الخاصه بي:  ${prefix}
...
⧉ - Dare تحدي / Truth الحقيقة

⧉  Truth 
البوت يعطيك سؤال محرج  ولازم تجاوب بصراحة ولو ما جاوبت تتعاقب
⧉ Dare 
البوت يعطيك تحدي مجبور تسويه ولو ماسويته تتعاقب/ موقفة
 
⧉ -اقترح
⧉  يقترح إنمي عشوائي/موقفة

⧉ -كت
⧉   موقفة/لعبة كتابة

⧉ -سؤال
⧉  يعطيك سؤال عشوائي / موقفة
...
*⚡ثم نشكر دابي سان على هاذا عمل عظيم⚡*

*⚡ثم أتمنى وأطلب ﷲ أن يعجبكم البوت وأن ينال إعجابكم وأستمحكم وأترككم من أجل أن تستمتعو بكل شيء وإلا القاء🤍⚡ .*
...
case 'رأفت': case 'جورج': case 'رايزل':
    if (isBan) return reply(mess.banned)	 			
    if (isBanChat) return reply(mess.bangc)

const txt = `شات رايزل / رقمي للتواصل معي: https://api.whatsapp.com/send?phone=+967770500831 / قروب الدعم: https://chat.whatsapp.com/CMp1e9NHmHU5K5sCM2mmKn=`
const mikuarray= [
